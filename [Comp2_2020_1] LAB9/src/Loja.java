...
public class Loja {
    ...
    private Map<Vendavel, Integer> quantidadeEmEstoquePorItem = new HashMap<>();
    ...
    public String receberPedido(Vendavel item, int quantidade, Usuario usuario)
        ...
        if (quantidadeEmEstoque < quantidade) {
            ...
            quantidadeEmEstoquePorItem.put(item, quantidadeEmEstoque-quantidade);
        }
        ...
        if (item instanceof Livro) {
            ...
            if (quantidade >=3 & ((Livro) item).getCategoria()==CategoriaLivro.Livro_Didático){
    ...
    public void incluirItem(Vendavel vendavel, Integer quantidadedeitemsnoestoque) {
        if (buscarItem(vendavel.getId()) != null) {
            // produto já existe no catálogo -- nada a fazer
            return;
        }
        catalogo.add(vendavel);
        quantidadeEmEstoquePorItem.put(vendavel, quantidadedeitemsnoestoque );
    }
    ...
    public int getQuantidadeEmEstoquePorItem(Vendavel vendavel){
        return quantidadeEmEstoquePorItem.get(vendavel);
    }

    public void setQuantidadeEmEstoquePorItem(Vendavel vendavel, Integer valor) {
        this.quantidadeEmEstoquePorItem.put(vendavel,quantidadeEmEstoquePorItem.get(vendavel)+ valor);
    }
